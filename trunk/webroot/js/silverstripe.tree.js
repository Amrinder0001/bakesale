/*
 * Country-seperated javascript tree widget
...
$(document).ready(function(){
	$('ul.tree li').addClass('closed');
	$('ul').each(function() {
		$(this).is('[class*="tree"]') && initTree(this); 
	});

	s = new String(window.location.href);
	url = s.replace('<?php echo FULL_BASE_URL; ?>','');
	$('ul.tree a[href="' +url +'"]').parents('li').removeClass('closed');
	$('ul.tree input[type="checkbox"]:checked').parents('li').removeClass('closed');
...
function initTree(el) {
	...
	if(li) {
		// li and spanA will still be set to the last product
...
function treeOpen( a, b ){
	$(a).removeClass('spanClosed');
	$(b).removeClass('closed');
}
	
	
function treeClose( a, b ){
	$(a).addClass('spanClosed');
	$(b).addClass('closed');
}
